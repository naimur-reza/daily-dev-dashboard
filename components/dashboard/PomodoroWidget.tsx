...
export default function PomodoroWidget({ sessionCount: initial, userId, today }: Props) {
  ...
  async function logSession() {
    await supabase.from("pomodoro_sessions").insert({ user_id: userId, date: today, minutes: 25 });
    setSessions((s) => s + 1);
  }
  ...
  return (
    <div className="bg-gray-900 border border-gray-800/60 rounded-xl p-5 flex flex-col items-center">
      <h2 className="text-sm font-medium text-gray-400 uppercase tracking-widest mb-5 self-start">
        Pomodoro
      </h2>
      ...
      <div className="relative w-32 h-32 mb-5">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-mono font-semibold text-white">{mins}:{secs}</span>
          <span className="text-xs text-gray-500 mt-0.5">{isBreak ? "break" : "focus"}</span>
        </div>
      </div>

      <div className="flex gap-2 mb-5">
        <button onClick={reset} className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-gray-400 transition-colors">
        ...
        <button onClick={skip} className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-gray-400 transition-colors">
